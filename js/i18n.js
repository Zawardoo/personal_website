...
(function initI18n() {
  const SK = {
    ...
    'test.about.me.p2':   'Predchádzajúcich 2,5 roka som strávil ako <strong>QA Tester</strong> v tej istej banke — testovanie SOAP/REST API, písanie regresných skriptov v SoapUI a Postman a <strong>mentorovanie 3 juniornych testerov</strong> počas onboardingu.',
  ...
  const cache = new Map(); // stores original EN text per element
  ...
  function applyLang(newLang) {
    ...
    document.querySelectorAll('[data-i18n-ph]').forEach(el => {
      ...
      if (!phCache.has(el)) {
        phCache.set(el, el.placeholder);
      }
      if (lang === 'sk' && SK[key]) {
        el.placeholder = SK[key];
      } else {
        el.placeholder = phCache.get(el);
      }
    });
